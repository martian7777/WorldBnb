...
const leadership = [
    ...
    { name: "Leila Nazari", role: "Chief Financial Officer", avatar: "LN", color: "from-[#FF6B4A] to-[#14B8A6]" },
        ...
        <>
            ...
            <section className="py-16 bg-white">
                <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                    <h2 className="text-2xl font-black text-slate-900 mb-8 text-center">FY 2025 Performance</h2>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        {financials.map((f) => (
                            <div key={f.label} className="bg-slate-50 rounded-2xl p-6 border border-gray-100 hover:shadow-lg transition-all hover:-translate-y-1">
                                <div className="text-sm text-gray-500 font-medium mb-2">{f.label}</div>
                                <div className="text-3xl font-black text-slate-900 mb-1">{f.value}</div>
            ...
            <section className="py-14 bg-slate-50">
                <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                    <h2 className="text-2xl font-black text-slate-900 mb-8">Quarterly Highlights</h2>
                        ...
                        {quarterlyHighlights.map((q) => (
                            <div key={q.quarter} className="bg-white rounded-2xl p-7 border border-gray-100 shadow-sm hover:shadow-md transition-all">
                                <div className="flex flex-col sm:flex-row sm:items-start gap-6">
                                    <div className="bg-[#1E3A8A] text-white rounded-xl px-5 py-3 text-center shrink-0">
                                        <div className="text-xs font-semibold text-gray-400">Quarter</div>
                                        <div className="font-black text-lg">{q.quarter}</div>
                                    </div>
                                    <div className="flex-1">
                                        <div className="grid grid-cols-2 gap-4 mb-4">
                                            <div><div className="text-xs text-gray-400 mb-0.5">Revenue</div><div className="font-black text-xl text-slate-900">{q.revenue}</div></div>
                                            <div><div className="text-xs text-gray-400 mb-0.5">Nights Booked</div><div className="font-black text-xl text-slate-900">{q.nights}M</div></div>
                                        </div>
            ...
            <section className="py-14 bg-white">
                <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-2 gap-10">
                    {/* Investors */}
                    <div>
                        <h3 className="text-xl font-black text-slate-900 mb-5">Our Investors</h3>
                        <div className="space-y-4">
                            {investors.map((inv) => (
                                <div key={inv.name} className="flex items-center gap-4 bg-slate-50 rounded-xl p-4 border border-gray-100">
                    ...
                    {/* Leadership */}
                    <div>
                        <h3 className="text-xl font-black text-slate-900 mb-5">Leadership Team</h3>
                        <div className="grid grid-cols-2 gap-4">
                            {leadership.map((l) => (
                                <div key={l.name} className="bg-slate-50 rounded-2xl p-5 border border-gray-100 text-center hover:shadow-md transition-all">
            ...
            {/* IR Contact */}
            <section className="py-12 bg-[#1E3A8A] text-center">
                <div className="max-w-xl mx-auto px-4">
                    ...
                    <a href="mailto:ir@Rentora.com" className="inline-block bg-white text-slate-900 font-bold px-8 py-3.5 rounded-2xl hover:bg-gray-100 transition-all">
                        ir@Rentora.com
                    </a>
                </div>
            </section>

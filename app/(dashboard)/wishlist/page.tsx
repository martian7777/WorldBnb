                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    {visible.map((listing) => (
                        <div key={listing.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-lg transition-all overflow-hidden group">
                            {/* Image placeholder */}
                            <div className="relative h-44 bg-gradient-to-br from-rose-50 to-pink-50 flex items-center justify-center">
                                <span className="text-6xl">{listing.image}</span>
                                {/* Remove from wishlist */}
                                <button
                                    onClick={() => setRemoved((prev) => [...prev, listing.id])}
                                    className="absolute top-3 right-3 w-8 h-8 bg-white rounded-full flex items-center justify-center text-rose-500 hover:text-gray-400 shadow-md transition-colors"
                                    aria-label="Remove from wishlist"
                                >
                                    ❤️
                                </button>
                                <span className="absolute top-3 left-3 bg-white text-gray-700 text-xs font-bold px-2.5 py-1 rounded-full">
                                    {listing.category}
                                </span>
                            </div>
                            {/* Info */}
                            <div className="p-5">
                                <h3 className="font-black text-gray-900 text-sm leading-snug mb-1 group-hover:text-rose-500 transition-colors">{listing.title}</h3>
                                <p className="text-xs text-gray-400 mb-3">{listing.location}</p>
                                <div className="flex items-center justify-between">
                                    <div>
                                        <span className="font-black text-gray-900">${listing.price}</span>
                                ...
                                <button className="w-full mt-4 bg-rose-500 hover:bg-rose-600 text-white font-bold text-sm py-2.5 rounded-xl transition-all">
                                    Book Now
                                </button>
                            </div>
                        </div>
